...
function createGroupName(series: SeriesEvent) {
  return `Workshop series | ${series.title.trim()} | ${series._id}`.slice(0, 255);
}
...
export const handler = documentEventHandler<SeriesEvent>(async ({context, event}) => {
  ...
  if (series.mailerLiteGroupId) {
    console.log(`Workshop series ${series._id} already has a MailerLite group.`);
    return;
  }
  ...
  try {
    ...
    const group = await findOrCreateGroup(createGroupName(series), apiKey);
